...
//    150 combinations for the given input.
    ...
    public static List<List<Integer>> combinationSum(int[] candidates, int target) {
        ...
        backtrack(candidates, target, rsl, new ArrayList<>(), 0);
        return rsl;
    }

    private static void backtrack(int[] candidates, int target, List<List<Integer>> rsl, List<Integer> comb, int start) {
        if (target == 0) {
        ...
        for (int i = start; i < candidates.length; i++) {
            ...
            backtrack(candidates, target - candidates[i], rsl, comb, i);
